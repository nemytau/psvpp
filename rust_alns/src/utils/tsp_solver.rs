use crate::structs::{
    constants::{DAYS_IN_PERIOD, HOURS_IN_DAY, HOURS_IN_PERIOD},
...
impl TSPSolver {
    ...
    pub fn new_from_problem_data(problem_data: &ProblemData) -> Self {
        ...
        let visit_to_installation: HashMap<usize, usize> = visits
            .iter()
    ...
    pub fn solve_and_get_end_time(&self, visit_ids: &[usize], speed: f64, start_time: f64) -> f64 {
        self.solve_internal(visit_ids.to_vec(), speed, start_time)
            .end_time
    }
    ...
    fn solve_internal(&self, visit_ids: Vec<usize>, speed: f64, start_time: f64) -> TSPResult {
        ...
        let (best_route, _) =
            self.solve_tsp_branch_and_bound(inst_ids.clone(), Some(speed), Some(start_time));
        ...
        let route_without_depot =
            self.installation_ids_to_visit_ids_sequence(&route_without_depot, &visit_ids);
        TSPResult {
    ...
    pub fn calculate_voyage_details_public(
        &self,
        route: &Vec<usize>,
        vessel_speed: f64,
        start_time: f64,
    ...
    fn visit_ids_to_installation_ids_sequence(&self, visit_ids: &[usize]) -> Vec<usize> {
        visit_ids
            .iter()
    ...
    fn installation_ids_to_visit_ids_sequence(
        ...
        for &inst_id in installation_ids {
            if let Some(pos) = remaining_visits
                .iter()
                .position(|&vid| self.visit_to_installation.get(&vid) == Some(&inst_id))
            {
    ...
    pub fn get_time_window(&self, node_id: usize) -> Option<(f64, f64)> {
        if node_id < self.daily_time_windows.len() {
            let tw = &self.daily_time_windows[node_id];
            Some((
                tw.earliest.unwrap_or(0.0),
                tw.latest.unwrap_or(HOURS_IN_DAY as f64),
            ))
        } else {
            None
        }
    ...
    fn visit_sequence_to_route(&self, visit_sequence: &Vec<usize>) -> Vec<usize> {
    ...
    fn inst_sequence_to_route(&self, inst_sequence: &Vec<usize>) -> Vec<usize> {
    ...
    fn calculate_voyage_details(
        ...
        // (sailing_time, waiting_time, arrival_time, end_time)
        ...
        for i in 0..route.len() - 1 {
            ...
            if next_node != 0 {
                // Calculate wait time
                let wait_time = self
                    .compute_wait_time(next_node, current_time)
                    .unwrap_or(0.0);
    ...
    pub fn solve_tsp_branch_and_bound(
        ...
        if inst_indices.iter().collect::<HashSet<_>>().len() != inst_indices.len() {
            eprintln!(
                "[ERROR] Duplicate installation IDs detected in TSP input: {:?}",
                inst_indices
            );
    ...
    pub fn compute_wait_time(&self, node: usize, arrival_time: f64) -> Option<f64> {
    ...
    pub fn analyze_route(
        ...
        result.push_str(&format!(
            "Route analysis (speed: {:.1}kn, start time: {:.1}h):\n",
            vessel_speed, start_time
        ));
        result.push_str(&format!(
            "Departure from depot (node {}) at time: {:.2}h\n",
        ...
        for i in 0..route.len() - 1 {
            ...
            if next_node != 0 {
                ...
                if wait_time > 0.0 {
                    result.push_str(&format!(
                        "Waiting for {:.2}h until time window opens\n",
                        wait_time
                    ));
                }
                ...
                result.push_str(&format!(
                    "Service time at node {}: {:.2}h\n",
                    next_node, service_time
                ));
                current_time += service_time;
                result.push_str(&format!(
                    "Departure from node {} at time: {:.2}h\n",
        ...
        result.push_str(&format!(
            "Total wait time at installations: {:.2}h\n",
            total_wait_time_at_installations
        ));
        result.push_str(&format!(
            "Total wait time at depot: {:.2}h\n",
            wait_time_at_depot
        ));
        result.push_str(&format!(
            "Total wait time: {:.2}h\n",
...
mod tests {
    ...
    use crate::structs::constants::{DAYS_IN_PERIOD, HOURS_IN_DAY};
    ...
    fn new_solver_with_single_node_and_tw(earliest: Option<f64>, latest: Option<f64>) -> TSPSolver {
        let distances = vec![vec![0.0, 24.0], vec![24.0, 0.0]];
    ...
    #[test]
    fn test_tsp_solver() {
        // Test the TSP solver implementation
        let distances = vec![
        ...
        let daily_time_windows = raw_tw_per_node
            .iter()
        ...
        let tsp_solver = TSPSolver::new(
            distances.clone(),
            daily_time_windows.clone(),
            service_times.clone(),
            visit_to_installation.clone(),
        );

        let (best_route, total_duration) =
            tsp_solver.solve_tsp_full_enumeration(inst_indices.clone(), None, None);
        let (bb_route, bb_duration) =
            tsp_solver.solve_tsp_branch_and_bound(inst_indices.clone(), None, None);
